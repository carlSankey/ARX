...
void processOpponentAction()
{
    ...
    {
		int braveness = Opponents[curOpponent].str + (50-Opponents[curOpponent].inte) + encounterQuantity + Opponents[curOpponent].hp + Opponents[curOpponent].braveness ;
		...
		else if (braveness < randn(0, ((plyr.str + plyr.level + plyr.inte) - plyr.alcohol))) // check to see if the oppoenent is scraed and flees
		{
			opponentFlees();
		}
		else if ((Opponents[curOpponent].callForHelp == 1) && (braveness < 20)) //Opponent can call for help
		{
			if (encounterQuantity < MAX_OPPONENTS) 
			{ 
				opponentCallsForHelp();
			} 
			else //if the oppenent doesn't call for help then they attach
			{
				opponentAttack();
			}
		} //Opponent can't call for help so they might snatch
		else if ((Opponents[curOpponent].snatch > 1) && calcSnatchSuccess())
		{
			int selected_index = snatchedItem( opponent.minLevel + 1 );
			if (selected_index == 0)
			{
				std::string selected_resource = selectRandomResource(plyr);
				if (selected_resource == "Nothing")
				{
					str = "The " + Opponents[curOpponent].name + " can't take anyhting as you have nothing of value ";
				}
				else
				{
					str = "The " + Opponents[curOpponent].name + " has taken some of your " + selected_resource;
				}
			}
			else
			{
				str = "The " + opponent.name + " has taken your " + itemBuffer[selected_index].name;
			}
			consoleMessage(str);
			updateDisplay();
		}
		else if ((Opponents[curOpponent].s0 > 0))
		{
			 castSpellAction(whichOpponentSpell(curOpponent));
    ...
    if (curOpponent==(encounterQuantity-1)) 
	 { 
	    ...
	    curOpponent++; 
		if(curOpponent>MAX_OPPONENTS-1 || curOpponent > encounterQuantity){ curOpponent = 0; }
...
void processPlayerAction()
{
	...
	//You are suprised
	else if (encounterMenu == 10)  //You are suprised
	{
...
void opponentFlees()
{
	...
	if ((curOpponent == 0 && encounterQuantity > 0) || (curOpponent >= 0 && curOpponent < encounterQuantity))
	{
			...
			else 
			{
				...
				else
				{
					for (int i = 0; i < encounterQuantity; i++) {
						...
						case 2:
							Opponents[i].stunnedTurnsRemaining = static_cast<int>(spells[spellNo].duration/4);
...
int NewcalcOpponentWeaponDamage(int weaponNo, float attackFactor, int attacker)
{
	...
	while (armorIndex < 11)
	{
		...
		noDice = static_cast<int>(std::round(noDice * numArmorPieces));
		noSides = static_cast<int>(std::round(noDice * numArmorPieces));
...
void checkEncounter()
{
	...
	int random_value = randn(0,100); // random number between 0 and 100

	// Calculate the bias from player's attributes
	float stealth_bias = plyr.stealth * 0.5f;  // Stealth increases the value
	float noticeability_bias = plyr.noticeability * -0.3f;  // Noticeability decreases the value
	float alcohol_bias = plyr.alcohol * -0.4f;  // Alcohol decreases the value

	// Calculate the total bias
	float total_bias = stealth_bias + noticeability_bias + alcohol_bias;

	// Apply the bias to the random value
	int biased_value = static_cast<int>(random_value + total_bias);

	// Clamp the result to ensure it's within the 0 to 100 range
	if (biased_value < 0) biased_value = 0;
	if (biased_value > 100) biased_value = 100;

	// Check if the encounter theme is not playing
	if (encounterThemeNotPlaying()) {
		// Use the biased value for further checks or logic
		int encounter_check = biased_value; // This is the result to use
	

		
		SurpriseLevel level = getSurpriseLevelEnum(encounter_check);
		cout << "encounter_check: " << encounter_check << " " << "\n";

		switch (level) {
		case Surprised:
			playerSurprised = true;
			std::cout << "Case Surprised executed.\n";
			chooseEncounter();
			break;
		case Noticed:
			std::cout << "Case Noticed executed.\n";
			chooseEncounter();
			break;
		case Neutral:
			std::cout << "Case Neutral executed.\n";
			break;
		case Observant:
			std::cout << "Case Observant executed.\n";
			chooseEncounter();
			break;
		case Dominant:
			opponentSurprised = true;
			std::cout << "Case Dominant executed.\n";
			chooseEncounter();
			break;
		default:
			std::cout << "No matching case found.\n";
			break;
		}
...
void chooseEncounter()
{
	...
	// CITY - Night
	else if ((plyr.timeOfDay==1) && (plyr.scenario==CITY))
	{
		int encCount = 0;
		int monsterProb = randn(0, 255);
	...
	else if ((plyr.scenario==DUNGEON) && ((plyr.zone == 17) || (plyr.zone == 16)))  // Dungeon - Well Lit Area
	{
		monsterNo = randomEncounterPicker(plyr.zone, plyr.timeOfDay, maps[plyr.map].encounterIndex);
	}
	else if ((plyr.scenario==DUNGEON) && !((plyr.zone == 17) || (plyr.zone == 16)))
    {
		monsterNo = randomEncounterPicker(plyr.zone, plyr.timeOfDay, maps[plyr.map].encounterIndex);
    }
	else if (plyr.scenario != DUNGEON || plyr.scenario != CITY)
	{
		monsterNo = randomEncounterPicker(plyr.zone, plyr.timeOfDay, maps[plyr.map].encounterIndex);
...
SurpriseLevel getSurpriseLevelEnum(int Surprisecheck) {
	if (Surprisecheck >= 0 && Surprisecheck <= 7) return Surprised;
	if (Surprisecheck >= 8 && Surprisecheck <= 20) return Noticed;
	if (Surprisecheck >= 21 && Surprisecheck <= 75) return Neutral;
	if (Surprisecheck >= 76 && Surprisecheck <= 92) return Observant;
	if (Surprisecheck >= 93 && Surprisecheck <= 100) return Dominant;

	// Optional: Handle out-of-range values
	throw std::out_of_range("Surprisecheck value is out of the expected range (0-100)");
}

bool calcSnatchSuccess() {
	// Calculate the combined strength and skill of both players
	int plyrScore = plyr.str + plyr.skl;
	int opponentScore = opponent.str + opponent.skl;

	// Ensure scores are non-negative to avoid invalid probability
	if (plyrScore < 0) plyrScore = 0;
	if (opponentScore < 0) opponentScore = 0;

	// Calculate a probability based on the relative scores
	double totalScore = plyrScore + opponentScore;
	double probability = (totalScore > 0) ? static_cast<double>(plyrScore) / totalScore : 0.5;

	// Introduce some randomness to allow success even if the opponent is stronger
	...
	// Return true if the random value is less than or equal to the calculated probability
	return randomValue <= probability;
}

 
int snatchedItem(int maxlevel) {
	double total_weight = 0.0;

	// Calculate total weight for eligible items
	for (int x = 0; x < plyr.items_index; x++) {
		if (itemBuffer[x].level <= maxlevel && itemBuffer[x].location == 10) {
			total_weight += itemBuffer[x].weight;
		}
	}

	if (total_weight == 0) {
		return 0; // No eligible items
	}
	...
	double rand_weight = dis(gen);

	// Perform weighted selection
	double cumulative_weight = 0.0;
	for (int x = 0; x < plyr.items_index; x++) {
		if (itemBuffer[x].level <= maxlevel && itemBuffer[x].location == 10) {
			cumulative_weight += itemBuffer[x].weight;

			if (rand_weight < cumulative_weight) {
				return itemBuffer[x].index; // Return the selected item's index
			}
		}
	}

	return 0; // Fallback in case no item is selected
}

std::string selectRandomResource(Player& plyr) {
	// Store resources and their names in parallel vectors
	std::string selected_name = "Nothing";
	std::vector<int*> resources = { &plyr.gold, &plyr.silver, &plyr.copper, &plyr.gems, &plyr.jewels };
	std::vector<std::string> resource_names = { "gold", "silver", "copper", "gems", "jewels" };

	// Filter for resources greater than 0
	std::vector<std::pair<int*, std::string>> valid_resources;
	for (size_t i = 0; i < resources.size(); ++i) {
		if (*resources[i] > 0) {
			valid_resources.push_back(std::make_pair(resources[i], resource_names[i]));
		}
	}

	// If no valid resources, throw an exception or handle it
	if (!valid_resources.empty()) {
		// Randomly select from the valid resources
		std::random_device rd;
		std::mt19937 gen(rd());
		std::uniform_int_distribution<> dis(0, valid_resources.size() - 1);

		auto selected_pair = valid_resources[dis(gen)];
		int* selected_resource = selected_pair.first;
		selected_name = selected_pair.second;

		// Generate a random reduction between 1 and the current value of the selected resource
		std::uniform_int_distribution<> reduction_dis(1, *selected_resource);
		int reduction = reduction_dis(gen);

		// Reduce the selected resource by the generated random number
		*selected_resource -= reduction;

		std::cout << "Reduced " << selected_name << " by " << reduction
			<< ". New value: " << *selected_resource << std::endl;
		return selected_name;
	}
	else
	{
		throw std::runtime_error("No resources are greater than 0.");
		return selected_name;
	}
...
{
	int chosenSpell = randn(1, 5);
		return chosenSpell;

}


int DamageOverTime(int category, int dotLevel)
{
