...
sf::Time dt;
sf::Time encounterCheckTime = sf::Time::Zero; // Accumulated time
...
sf::Sound cityDoorSound{ doorCityBuffer };
sf::Sound citySecretSound{ secretCityBuffer };
sf::Sound dungeonDoorSound{ secretDungeonBuffer };
sf::Sound dungeonSecretSound{ doorDungeonBuffer };

sf::SoundBuffer smithyBuffer;
sf::Sound smithySound{ smithyBuffer };
bool smithyPlaying;
sf::Music deathMusic;
bool gameQuit;
...
std::optional<sf::Sprite> tBackground;
...
void gameLoop()
{
    ...
    while (Running)
    {
		...
		sf::Time dt = myclock.restart(); // Time since last frame
		...
		updateDisplay();
		// Clamp delta time to avoid large spikes
		if (dt.asSeconds() > 0.1f) {
			dt = sf::seconds(0.1f); // Cap to 100ms
		}
		    encounterCheckTime += dt;
		    if (encounterCheckTime >= sf::seconds(4.8f) && plyr.status != ENCOUNTER) // 4.8f// was 0.8f
			{
				
				std::cout << "Delta time: " << dt.asSeconds()
					<< ", Encounter time: " << encounterCheckTime.asSeconds() << std::endl;
				plyr.status_text = " ";
		...
		while (const std::optional Event = App.pollEvent()) {
			{
				...
				{
					if (keyPressed->code == sf::Keyboard::Key::Left) turnLeft();
					if (keyPressed->code == sf::Keyboard::Key::Right) turnRight();
					if (keyPressed->code == sf::Keyboard::Key::Up) moveForward();
