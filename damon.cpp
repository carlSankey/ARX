...
while (damonMenu == 22) // buy item?
{
    ...
    damonMenu = 23;
		...
		while (damonMenu == 4) // buy item?
		{
			itemNo = menuStartItem+itemChoice;
			itemCost = newItemArray[newdamonBattleGearWares[itemNo].itemRef].cost;
			float tempitemcost = static_cast<float>(newItemArray[newdamonBattleGearWares[itemNo].itemRef].cost);
			float temp = (tempitemcost / 100) * 75;
			itemLowestCost = static_cast<int>(temp);
			damonOffer = itemCost;
			damonMenu = 18;
...
{
	int index, alignment, weight,  melee, ammo, blunt, sharp, earth, air, fire, water, power, magic, good, evil, cold, nature, acid,
