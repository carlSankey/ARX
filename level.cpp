...
//Looks like another thing that needs creating outside
Teleport teleports[33] = {
...
void initMaps()
{
	...
	while (junk!="EOF")	
	{
		...
		for (int a = 0; a < fields; ++a) // number of attributes - test for 33
		{
			...
			if (a == 5) { maps[i].scenarioNumber = atoi(text.c_str()); }
